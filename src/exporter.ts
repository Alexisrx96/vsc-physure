...
/**
 * Checks whether a line of MKML code contains a variable assignment operator (`=`).
...
/**
 * Replaces quantity literals like `1.673e-27 kg` or `10 m +/- 0.5 m` in an MKML
...
/**
 * Transpiles a single MKML code line or expression into native Python code.
 */
...
/**
 * Smartly transpiles MKML source code into clean, native Python code
 * using `Q_` and native Python constructs (functions, expressions, variables).
 */
export function transpiledMkmlToPython(mkmlText: string): string {
    const lines = mkmlText.split(/\r\n|\r|\n/);
...
/**
 * Generates a clean Markdown report template from MKML source.
 * Pure function independent of VS Code API.
 */
export function mkmlToMarkdownReport(fileName: string, mkmlText: string): string {
    const lines = mkmlText.split(/\r\n|\r|\n/);
